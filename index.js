	...
	methods: {
		connect(ctx) {
			return this.adapter.connect().then(() => {
				if (
					(0, lodash_isfunction_1.default)(this.schema.afterConnected)
				) {
					try {
		...
		async transformDocuments(params, ctx, docs) {
			...
			if (!Array.isArray(docs)) {
				if ((0, lodash_isobject_1.default)(docs)) {
					isDoc = true;
					docs = [docs];
				} else return bluebird_1.default.resolve(docs);
			}
		...
		filterFields(doc, fields) {
			if (Array.isArray(fields)) {
				...
				fields.forEach((n) => {
					...
					if (v !== undefined) (0, lodash_set_1.default)(res, n, v);
				});
				return res;
			}
			return doc;
		...
		async _update(ctx, meta) {
			...
			Object.keys(meta).forEach((prop) => {
				...
				else throw new errors_1.FileNotFoundError('No valid ID');
			});
			...
			return this.adapter.removeById(id).then((doc) => {
				if (!doc)
					return bluebird_1.default.reject(
						new errors_1.FileNotFoundError(String(params.id)),
					);
	...
	created() {
		...
		if (
			...
			this.settings.entityValidator = (entity) => {
				...
				if (res === true) return bluebird_1.default.resolve();
				else
					return bluebird_1.default.reject(
						new ValidationError(
